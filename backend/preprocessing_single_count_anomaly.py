...
def LSTM_MODEL():
    ...
    return plot_data_json, count, arr_anom_dates
...
plotGraph, count, anomaly_dates = LSTM_MODEL()

# Convert the 'ds' column to datetime type
count_data['ds'] = pd.to_datetime(count_data['ds'])

# Match the timestamps and retrieve the rows
result_dict = count_data[count_data['ds'].isin(anomaly_dates)]

# Convert DataFrame to dictionary
count_pieChart = {'time': result_dict['ds'].tolist(), 'count': result_dict['y'].tolist()}
print(count_pieChart)
...
data_send1 = {
    'name': plotGraph,
    'count': count,
    'taskCategoryOverall': taskCategoryOverall,
    'eventperday': eventperday,
    'piechartCount': count_pieChart
}

if user_data1:
    list_to_be_return1 = user_data1.to_son().to_dict()
    list_to_be_return1['list_to_be_return'][user]['count'] = data_send1
    list_to_be_return[user] = list_to_be_return1['list_to_be_return'][user]

else:
    data_send['count'] = data_send1
    list_to_be_return[user] = data_send

ModelData.add_data(user, list_to_be_return=list_to_be_return)
