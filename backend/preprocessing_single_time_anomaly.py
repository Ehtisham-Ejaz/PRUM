...
def LSTM_MODEL():
    ...
    return plot_data_json, totalAnom, arr_anom_dates
...
plotGraph, count, anomalyDates = LSTM_MODEL()

taskCategoryOverall = result_for_total_category(cols)
eventperday = result_for_events_per_day()

# Extract date and time components
time_list = [ts.strftime('%H:%M:%S') for ts in anomalyDates]
date_list = [ts.strftime('%Y-%m-%d') for ts in anomalyDates]

# Create the dictionary
time_barChart = {'time': time_list, 'date': date_list}
...
data_send1 = {
    ...
    'bargraphTime': time_barChart
}

if user_data1:
    list_to_be_return1 = user_data1.to_son().to_dict()
    list_to_be_return1['list_to_be_return'][user]['time'] = data_send1
    list_to_be_return[user] = list_to_be_return1['list_to_be_return'][user]

else:
    data_send['time'] = data_send1
